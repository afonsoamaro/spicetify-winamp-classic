...
- **What the DOM actually exposes on this bar**, learned the expensive way and worth keeping for the FN issues:
  - Container testids render (`player-controls`, `playback-progressbar`, `now-playing-widget`), per-button ones do not. `[data-testid="control-button-shuffle"]` matches nothing, and neither does `aria-checked`, even though both appear as props in the bundle.
  - The toggled state of shuffle and repeat is a generated class per control, with no readable name in Spicetify's class map. Repeat carries it on its `button`; shuffle carries it on an inner element, so the rule matches on the class alone.
  - Spotify's stylesheets load after the theme, so ties on specificity go to Spotify. Scoping every rule under the bar wins those without reaching for `!important`.
  - The way to find any of this is the class map at `~/.spicetify/css-map.json` first, then the bundle under `Apps/xpui/`. Probing the screen to guess a selector costs a restart per attempt and answers nothing the bundle does not already say.
- **Repeat lights up when it is on; shuffle does not, and that is open.** Repeat carries a generated class on its own button, so the key presses in and the icon turns green. Shuffle marks itself somewhere the theme could not reach: its per-button testid and `aria-checked` never render, the wrapper span that draws its dot is not matched from the controls container, and Spicetify's class map is stale here, mapping `main-shuffleButton-active` and `main-repeatButton-active` to hashes that no longer exist in 1.2.99.317. Older Marketplace themes style `main-shuffleButton-on`, a name this build dropped as well.
- **An idea worth trying before hunting for the class again:** the idle icon colour is forced with a rule more specific than Spotify's own active-state rule, which is why the accent never shows. Lowering the specificity of the idle rule below Spotify's would let the active colour win on its own, with no state class to find and nothing to re-read after a Spotify update.
